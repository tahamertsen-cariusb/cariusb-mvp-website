'use client';

import { useState, useEffect } from 'react';
import Link from 'next/link';
import { Navbar } from '@/components/layout/Navbar';
import { Footer } from '@/components/layout/Footer';
import { useAuthStore } from '@/stores/authStore';
import styles from './page.module.css';

interface ConnectedApp {
  name: string;
  icon: JSX.Element;
  connected: boolean;
  username?: string;
}

const connectedApps: ConnectedApp[] = [
  {
    name: 'GitHub',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
    ),
    connected: true,
    username: '@alexjohnson',
  },
  {
    name: 'Twitter',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
        <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
      </svg>
    ),
    connected: false,
  },
  {
    name: 'LinkedIn',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
      </svg>
    ),
    connected: false,
  },
];

export default function SettingsPage() {
  const { isLoggedIn } = useAuthStore();
  const [darkMode, setDarkMode] = useState(true);
  const [language, setLanguage] = useState('English');
  const [outputQuality, setOutputQuality] = useState('High (1536px)');
  const [emailNotifications, setEmailNotifications] = useState(true);
  const [marketingEmails, setMarketingEmails] = useState(false);
  const [renderAlerts, setRenderAlerts] = useState(true);
  const [passwordData, setPasswordData] = useState({
    current: '',
    new: '',
    confirm: '',
  });

  return (
    <>
      <Navbar />
      <main className={styles.pageContainer}>
        <div className={styles.pageHeader}>
          <h1 className={styles.pageTitle}>Account Settings</h1>
          <p className={styles.pageSubtitle}>Manage your profile and preferences</p>
        </div>

        {/* Settings Navigation */}
        <div className={styles.settingsNav}>
          <Link href="/profile" className={styles.settingsNavItem}>
            Profile
          </Link>
          <Link href="/settings" className={`${styles.settingsNavItem} ${styles.active}`}>
            Settings
          </Link>
          <Link href="/billing" className={styles.settingsNavItem}>
            Billing
          </Link>
        </div>

        {/* Preferences */}
        <div className={styles.settingsSection}>
          <h2 className={styles.sectionTitle}>Preferences</h2>
          <p className={styles.sectionSubtitle}>Customize your experience</p>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Dark Mode</h4>
              <p>Use dark theme across the application</p>
            </div>
            <div
              className={`${styles.toggleSwitch} ${darkMode ? styles.active : ''}`}
              onClick={() => setDarkMode(!darkMode)}
            ></div>
          </div>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Language</h4>
              <p>Select your preferred language</p>
            </div>
            <select
              className={styles.settingSelect}
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
            >
              <option>English</option>
              <option>Türkçe</option>
              <option>Deutsch</option>
              <option>Español</option>
            </select>
          </div>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Default Output Quality</h4>
              <p>Quality setting for generated images</p>
            </div>
            <select
              className={styles.settingSelect}
              value={outputQuality}
              onChange={(e) => setOutputQuality(e.target.value)}
            >
              <option>High (1536px)</option>
              <option>Ultra (2048px)</option>
              <option>Standard (1024px)</option>
            </select>
          </div>
        </div>

        {/* Notifications */}
        <div className={styles.settingsSection}>
          <h2 className={styles.sectionTitle}>Notifications</h2>
          <p className={styles.sectionSubtitle}>Choose what you want to be notified about</p>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Email Notifications</h4>
              <p>Receive updates about your projects via email</p>
            </div>
            <div
              className={`${styles.toggleSwitch} ${emailNotifications ? styles.active : ''}`}
              onClick={() => setEmailNotifications(!emailNotifications)}
            ></div>
          </div>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Marketing Emails</h4>
              <p>News, updates, and promotional content</p>
            </div>
            <div
              className={`${styles.toggleSwitch} ${marketingEmails ? styles.active : ''}`}
              onClick={() => setMarketingEmails(!marketingEmails)}
            ></div>
          </div>

          <div className={styles.settingRow}>
            <div className={styles.settingInfo}>
              <h4>Render Complete Alerts</h4>
              <p>Get notified when your renders are ready</p>
            </div>
            <div
              className={`${styles.toggleSwitch} ${renderAlerts ? styles.active : ''}`}
              onClick={() => setRenderAlerts(!renderAlerts)}
            ></div>
          </div>
        </div>

        {/* Security */}
        <div className={styles.settingsSection}>
          <h2 className={styles.sectionTitle}>Security</h2>
          <p className={styles.sectionSubtitle}>Keep your account secure</p>

          <div className={styles.passwordGrid}>
            <div className={styles.formGroup}>
              <label className={styles.formLabel}>Current Password</label>
              <input
                type="password"
                className={styles.formInput}
                placeholder="Enter current password"
                value={passwordData.current}
                onChange={(e) => setPasswordData({ ...passwordData, current: e.target.value })}
              />
            </div>
            <div className={styles.formGroup}>
              <label className={styles.formLabel}>New Password</label>
              <input
                type="password"
                className={styles.formInput}
                placeholder="Enter new password"
                value={passwordData.new}
                onChange={(e) => setPasswordData({ ...passwordData, new: e.target.value })}
              />
            </div>
            <div className={styles.formGroup}>
              <label className={styles.formLabel}>Confirm New Password</label>
              <input
                type="password"
                className={styles.formInput}
                placeholder="Confirm new password"
                value={passwordData.confirm}
                onChange={(e) => setPasswordData({ ...passwordData, confirm: e.target.value })}
              />
            </div>
            <button className={styles.btnPrimary}>Update Password</button>
          </div>
        </div>

        {/* Connected Apps */}
        <div className={styles.settingsSection}>
          <h2 className={styles.sectionTitle}>Connected Apps</h2>
          <p className={styles.sectionSubtitle}>Manage third-party integrations</p>

          {connectedApps.map((app) => (
            <div key={app.name} className={styles.connectedApp}>
              <div className={styles.appInfo}>
                <div className={styles.appIcon}>{app.icon}</div>
                <div className={styles.appDetails}>
                  <h4>{app.name}</h4>
                  <p>{app.connected ? `Connected as ${app.username}` : 'Not connected'}</p>
                </div>
              </div>
              {app.connected ? (
                <button className={styles.btnDisconnect}>Disconnect</button>
              ) : (
                <button className={styles.btnConnect}>Connect</button>
              )}
            </div>
          ))}
        </div>
      </main>
      <Footer />
    </>
  );
}
